...
\begin{enumerate}
    ...
    while x < 10 do
        if x is even then
            x <- x + 3 # remember that a 0 is even :)
        else
            x <- x + 1
    \end{verbatim}
    \newpage
    ...
    \begin{enumerate}
        ...
        \vspace{5cm}
        \item Find the \textit{signed} area under the curve of $g$ from $x = 3$ to $x = 6$ 
        \vspace{5cm}
        \item Find the \textit{unsigned} area between the curve and $x$ axis over $g$ from $x = 3$ to $x = 6$ 
        \vspace{5cm}
    \end{enumerate}
    ...
    \begin{enumerate}
    \item How many different ways can you arrange the letters in the word "METHODS"? 
    \vspace{3cm}
